...
const App = () => {
    ...
    const generateRecommendations = async (mealsCaloriesPerc, totalCalories) => {
        ...
        for (let meal in mealsCaloriesPerc) {
            ...
            const nutrition = getRandomNutrition(mealCalories);
            try {
                const response = await axios.post('/api/pickle-data', { nutrition });
                recommendations.push(response.data.output);
            } catch (error) {
                console.error(`Error generating recommendations for ${meal}:`, error);
                // Handle error
            }
        }
        return recommendations;
    ...
    return (
        <div className="container">
            ...
            {calorieResult && (
                <div>
                    <h2>Calories Calculator</h2>
                    <p>Maintain Calories: {calorieResult.toFixed(2)} Calories/day</p>
                </div>
            )}
            {recommendations.length > 0 && (
                <div>
